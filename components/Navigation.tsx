...
const Navigation: React.FC<NavigationProps> = ({ onVent, onReset, onSOS, onBreathe, onMood }) => {
  return (
    <aside className="fixed top-0 left-0 w-full h-16 md:h-full md:w-20 z-50 flex md:flex-col justify-between items-center p-3 md:py-8 bg-bg/90 md:bg-bg/80 backdrop-blur-lg border-b md:border-b-0 md:border-r border-glass-border transition-all">
      {/* Logo */}
      <div className="text-white/30 font-title font-extrabold text-lg md:text-2xl tracking-widest md:[writing-mode:vertical-rl] md:rotate-180 select-none hidden md:block">
        IYM PSICO
      </div>
      <div className="md:hidden text-white/30 font-title font-extrabold text-lg">IYM</div>

      {/* Buttons */}
      <div className="flex md:flex-col gap-2 md:gap-4 overflow-x-auto md:overflow-visible w-full md:w-auto justify-end md:justify-start pr-2 md:pr-0">
        
        <button 
          onClick={onMood}
          className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-white/5 border border-glass-border flex items-center justify-center text-xl text-pink-200 hover:bg-pink-500/20 hover:text-pink-400 hover:border-pink-400 active:scale-95 transition-all duration-300 touch-manipulation shrink-0"
